            ...
            <div class="mb-8 flex gap-4 justify-center">
                ...
                @foreach($categories as $cat)
                    <a href="/?category={{ $cat->slug }}"
                        class="px-5 py-2 {{ request('category') == $cat->slug ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-700' }} rounded-xl font-bold hover:bg-indigo-600 hover:text-white transition">
                        {{ $cat->name }}
                    </a>
                @endforeach
            </div>
